...
import fs from 'fs';
import { getDatabaseConfig, backupDatabase, restoreDatabase } from './db-config';

const config = getDatabaseConfig();
console.log('🗄️ Database config:', config);

// Try to restore from backup if main database doesn't exist but backup does
if (!fs.existsSync(config.path) && config.backupPath && fs.existsSync(config.backupPath)) {
  console.log('🔄 Restoring database from backup...');
  restoreDatabase(config.backupPath, config.path);
}
